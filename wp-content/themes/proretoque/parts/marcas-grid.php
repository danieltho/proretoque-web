...
?>
<div class="marcas-grid__container">
    <div class="marcas-grid">
        <?php $index = 0; $total = $marcas->post_count; ?>
        <?php while ( $marcas->have_posts() ) : $marcas->the_post();
            $pos_in_cycle = $index % 4; // 0=large, 1=small, 2=small, 3=large
            $is_large     = ( $pos_in_cycle === 0 || $pos_in_cycle === 3 );
            $size_class   = $is_large ? 'marcas-grid__item--large' : 'marcas-grid__item--small';

            // If a small item is alone in its row (no large companion), it needs its own height.
            $alone_in_row = ( ! $is_large && $pos_in_cycle === 2 && $index === $total - 1 );
            if ( $alone_in_row ) {
